...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : Main program body for LCD, DHT11, Bluetooth module
  ******************************************************************************
  * @attention
  *
...
uint8_t btchar;
char btData[50];
volatile unsigned int tim3MinFlag;
/* USER CODE END PV */
...
int main(void)
{
  ...
  LCD_writeStringXY(0, 0, "welcome home");
  ...
  while (1)
  {
	  if(tim3MinFlag) //10 sec 마다 온습도데이터를 읽은 후, LCD에 출력
	  {
		  dht11Data = DHT11_readData();
		  sprintf(buff,"h: %d%% t: %d.%d'C", dht11Data.rh_byte1, dht11Data.temp_byte1, dht11Data.temp_byte2);
		  LCD_writeStringXY(1, 0, buff);
		  sprintf(buff, "[room1]%d:%d.%d", dht11Data.rh_byte1, dht11Data.temp_byte1, dht11Data.temp_byte2);
		  printf("%s\r\n", buff);
